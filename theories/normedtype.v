...
Lemma nonneg_leq (x y : {nonneg R}): (x%:nnnum <= y%:nnnum) = (x <= y).
Proof. by []. Qed. 
...
Proof.
by rewrite nonneg_leq ; apply: ler_mx_norm'_add.  
Qed.
...
Proof.
rewrite {1 3}/normr /= !mx_normE (eq_bigr (fun i => `|l| * `|x i.1 i.2|)); last first.
...
rewrite [in X in _ <= X]/normr /= mx_normE.
